...
exports.config = {
  ...
  onPrepare: async () => {
    ...
    /**
     * The test status needs to be updated on Sauce. That can be done with the following
     */
    jasmine.getEnv().addReporter({
      specDone: async (result) => {
        await browser.executeScript(`sauce:job-result=${result.failedExpectations.length === 0}`);
      },
    });
  },
};
