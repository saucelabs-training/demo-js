...
# Android
curl \
  -F "payload=@../apps/Android.SauceLabs.Mobile.Sample.app.apk" \
  -F name=sample-app-android.apk \
  -u "$SAUCE_USERNAME:$SAUCE_ACCESS_KEY"  \
  'https://api.us-west-1.saucelabs.com/v1/storage/upload'
# iOS
curl \
  -F "payload=@../apps/iOS.Simulator.SauceLabs.Mobile.Sample.app.zip" \
  -F name=sample-app-ios.zip \
  -u "$SAUCE_USERNAME:$SAUCE_ACCESS_KEY"  \
  'https://api.us-west-1.saucelabs.com/v1/storage/upload'
curl \
  -F "payload=@../apps/iOS.RealDevice.SauceLabs.Mobile.Sample.app.ipa" \
